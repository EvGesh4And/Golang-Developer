...
func main() {
	...
	for {
		...
		var response *pb.SubmitVoteResponse
		if response, err = client.SubmitVote(ctx, req); err != nil {
			log.Println(err)
			continue
		}

		log.Printf("vote submitted %+v", response.GetIds())
	}
}
...
func getRequest(reader *bufio.Reader) (*pb.SubmitVoteRequest, error) {
	...
	if err != nil {
		return nil, errors.New("[client] wrong input, try again")
	}
