...
Если блокировка записи активна (rwm.Lock()), то все новые попытки заблокировать запись или чтение будут заблокированы до тех пор, пока запись не будет разблокирована (rwm.Unlock()).

Если блокировка чтения активна (rwm.RLock()), то:
...
Оптимизация с использованием sync.RWMutex
